pipeline {
     ...
     stage('Push image in staging and deploy it') {
      ...
      steps {
          script {
            sh '''
              curl -X POST http://${STG_API_ENDPOINT}/staging -H 'Content-Type: application/json' -d '{"your_name": ulrich,"container_image":choco1992/alpinehelloworld:latest, "external_port":80, "internal_port":5000}'
            '''
          }
        }
     }
